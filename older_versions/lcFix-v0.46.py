...
def getOptions():
    ...
    usageStr = """%prog [-h] [-v] [-o outputfile] [--dryrun] inputfile
    	...
    	outputs to root-fix.ext, where the inputfile name was root.ext
    	outputs time tear lines (if any) to root-timetears.txt
 ...
 Recommendations:
	...
	- After running, and once all time tears have been hand-corrected,
    	  re-run %prog --dryrun on the fixed file to verify that there are no
    	  remaining errors.  Add the "-v" option to get information about
    	  the directory header.  
    	  
Example: 
	- for an LCHEAPO file named RR38.lch:
    	  	> %prog RR38.lch  > RR38-fix.txt
    	  	> %prog -v --dryrun RR38-fix.lch > RR38-verify.txt
"""
    ...
    parser.add_option("-o", "--outfile", dest="outfile",
    				help="Force output (fixed) file name")
    ...
    if opt.dryrun:
    	if len(arguments) != 1:
        	print("\n%s Error: Program requires an inputfile only (DRYRUN)." % PROGRAM_NAME)
        	parser.print_help()
        	sys.exit(-1)
    else:
    	if len(arguments) != 2:
...
# Notes         : -
##############################################################################
def main(inFilename, opts, commandQ=None, responseQ=None):
	...
	if not dryrun:
		if opts.outfile is None :
			opts.outfile=root+"-fix"+ext
		ofp1 = open(opts.outfile, 'w')
	oftt = open(root+'-timetears.txt','w');
	...
	print("Finished at block %d (%d 1-sec errors, %d bad time, %d time tear, %d unexpected header values)" % 
	...
	for i in range(0, lcHeader.dirCount):
		...
		if lcDir.blockNumber > lastBlock :
			...
			if not dryrun:
				...
				print("IF THIS WASN'T A DRY RUN, WOULD REDUCE THE UMBER OF DIRECTORY ENTRIES FROM %d TO %d" % (lcHeader.dirCount,i))
			print("QUITTING")
			break
	...
	if countErrorTear>0:
		...
		print('CONGRATULATIONS, YOU HAVE NO TIME TEARS!!!!')
...
if __name__ ==  '__main__':
    ...
    main(arg[0], opt, commandQ, responseQ)
