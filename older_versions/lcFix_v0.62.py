...
VERSIONS = """    VERSIONS
    ...
    0.62:    
      - Corrected bug with "warnings" variable
    NOT YET:
...
def makeProcessStepsFile(startTimeStr,parameters,inFiles,outFiles,msgs,returnCode) :
  application={'comment':"Fixes common bugs in LCHEAPO data files",
    'name':"lcFix.py",
...
def main():
  ...
  for iFile in range(numInFiles):
    fname=args.infiles[iFile]
    ifp1 = open(fname, 'r')
    
    # Find and copy the disk header
    if iFile==0:    # First file, extract header
      hasHeader=True
      (lcHeader,firstBlock)=readLCHeader(ifp1)
      if args.verbosity:
    ...
    lastBlock=ifp1.tell()/512 - 1
    
    if stopProcess(commandQ):
      return

    # Adjust first block to correspond to first block with channel 0
    firstBlock=findFirstMux0Block(firstBlock,ifp1)
    
    outFileRoot=makeOutFileRoot(fname,numInFiles,args.outFileRoot,ifp1,firstBlock)
  
    # Process file
    (nBug1,nBug2,nTT,nWH,msg,ofname) = processInputFile(ifp1,fname,outFileRoot,lcHeader, firstBlock, lastBlock, hasHeader, args,commandQ, responseQ)
    ifp1.close()
...
##############################################################################
def processInputFile(ifp1, fname, outFileRoot, lcHeader, firstBlock, lastBlock, hasHeader, args, commandQ=None, responseQ=None):
  ...
  if not args.dryrun:
   ...
   #if verbosity:
  logging.info ("  data Blocks: first=%d, last=%d" % (firstBlock, lastBlock ))
  ...
  for i in range(firstBlock, lastBlock+1):
    ...
    if (i%5000 == 0):
      ...
      if stopProcess(commandQ):
        return
      if responseQ:
        responseQ.put((i, lastBlock, countErrorTimeBug1, countErrorTear))
  ...
  if quitEarly:
    logging.warning("  SHOULD CHANGE THE NUMBER OF DIRECTORY ENTRIES (on seekBlock in next Part?)")
    warnings+=1
  ...
  while True:
    if hasHeader:
      if iDir < lcHeader.dirCount:
        ...
        if iDir<=lcHeader.dirSize and nextDirBlock <= lastBlock :
    ...
    if hasHeader or not args.dryrun:  # ifp2 has a header
      lcData2.seekBlock(ifp2, lcDir.blockNumber)
    else: # ifp2 is headerless
      lcData2.seekBlock(ifp2, lcDir.blockNumber-lcHeader.dataStart)
  ...
  if iDir != lcHeader.dirCount :
    if hasHeader:
      if iDir < lcHeader.dirCount :
        logging.info("\n  DIR%d, BLOCK%d IS BEYOND THE END OF FILE!" % (iDir+1,lcDir.blockNumber))
